...
public class ProjectTwoMultiThread{
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int i = 3; i <= NUM_RANGE + 1; i++) {
            ...
            System.out.println("*********************** set size: " + i + " ***********************  free mem: " + Runtime.getRuntime().freeMemory() + "\n");
    ...
    private static ArrayList<int[]> getAndValidatePairs(ArrayList<Integer> numFreqResult) throws IOException {
        ...
        ArrayBlockingQueue<int[]> candidatePairList = new ArrayBlockingQueue<>(QUEUE_SIZE);
        ...
        ArrayList<int[]> result = validateFreqset(pairSize, new ArrayList<>(candidatePairList));

        // long end = System.currentTimeMillis();
        //long runTime = end - start;

        //System.out.println("total get and validate pairs, time: " + runTime + " ms, total freq num: " + numFreqResult.size());
    ...
    private static ArrayList<int[]> validateFreqset(int setSize, ArrayList<int[]> candidatePairList)
        ...
        //Supplier<Stream<byte[]>> rawNumArraySupplier = () -> Arrays.stream(rawNumArray);

        for (int[] oneCandSet : candidatePairList) {
            int matchCount = 0;
            matchCount = (int) Arrays.stream(rawNumArray).parallel()
                    .mapToInt(oneRawRow -> {
                        int counter = 0;
                        ...
                        //System.out.println("set size: " + setSize + ", validate result: " + ((counter == setSize) ? true: false));

                        return (counter == setSize) ? 1 : 0;
                    }).sum();

            //System.out.println("set size: " + setSize + ", array: " + Arrays.toString(oneCandSet) + ", curmatchcount: " + matchCount);
        ...
        System.out.println("validate freq sets, time: " + runTime + " ms, total cand sets: " + candidatePairList.size() + ", validated sets: " + result.size());
